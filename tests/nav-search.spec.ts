...
test.describe("NavSearch Component", () => {
  ...
  test("should not show dropdown for queries less than 3 characters", async ({ page }) => {
    const searchInput = page.getByPlaceholder("Search cities and areas...");
  ...
  test("should show search results for valid query", async ({ page }) => {
    ...
    await expect(page.getByRole("option").last()).toContainText("São José dos Campos");
  });
  ...
  test("should navigate to area page when pressing Enter on selected item", async ({ page }) => {
    const searchInput = page.getByPlaceholder("Search cities and areas...");
  ...
  test("should navigate to area page when clicking on result", async ({ page }) => {
    const searchInput = page.getByPlaceholder("Search cities and areas...");
  ...
  test("should close dropdown and clear search when pressing Escape", async ({ page }) => {
    const searchInput = page.getByPlaceholder("Search cities and areas...");
  ...
  test("should close dropdown when clicking outside", async ({ page }) => {
    ...
    // Dropdown should be hidden but input should keep its value
    await expect(page.getByRole("listbox")).toBeHidden();
    await expect(searchInput).toHaveValue("são");
  });
