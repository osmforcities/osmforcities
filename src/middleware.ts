...
export default auth((req) => {
  ...
  if (isLoggedIn) {
    ...
    console.log(
      `[MIDDLEWARE] User logged in, preferred language: ${preferredLanguage}`
    );
    ...
    if (preferredLanguage && routing.locales.includes(preferredLanguage)) {
      ...
      console.log(
        `[MIDDLEWARE] Current locale: ${currentLocale}, preferred: ${preferredLanguage}`
      );
      ...
      if (currentLocale && currentLocale !== preferredLanguage) {
        ...
        console.log(
          `[MIDDLEWARE] Redirecting from ${pathname} to ${newPathname}`
        );
        ...
        if (isValidLocalePath(newPathname)) {
          ...
          redirectResponse.headers.set(
            "X-Debug-Redirect",
            `locale-redirect:${currentLocale}->${preferredLanguage}`
          );
          return addDebugHeaders(redirectResponse);
        } else {
        ...
        console.log(
          `[MIDDLEWARE] Adding locale to path: ${pathname} -> ${redirectPath}`
        );
